...
- `revertVerifier`: extension-executed rollback check (never run through the agent). At `multiloop_decide` with action `revert` the extension runs it in its own process, parses its numeric output, and compares it against the pre-iteration value the rollback must restore (same `1e-6` relative tolerance). A passing `revert-verifier` check is appended to the iteration's checks; a failing one refuses the revert — the workspace must actually be restored to its pre-iteration state, on the extension's word, not the agent's. Pinned like the other verifier fields.
