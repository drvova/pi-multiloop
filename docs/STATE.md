...
- `lastWorkspaceFingerprint`: the workspace boundary fingerprint as of the last recorded decision (keep/revert/log) or loop start. The next `multiloop_iterate` captures the fingerprint again before changes are made and refuses to start if it differs — edits outside an iteration boundary would otherwise make the revert fingerprint capture an already-dirty baseline, so a later revert would be verified against the wrong state. With a `revertVerifier` configured, the verifier command defines the fingerprint and a failure clears the boundary (fingerprint unavailable). Without one, the built-in fingerprint `git status --porcelain --untracked-files=no` is used (best-effort: no git repository means no boundary, not a failure) — it covers the tracked working-tree state only, so untracked/ignored artifacts and commits are outside it; if a loop needs those covered, configure a `revertVerifier` with a custom command. The boundary covers the entire working tree, so concurrent loops sharing the worktree trip each other's drift gates — run loops sequentially, or give each lane a scoped `revertVerifier`. An explicit revert records the pre-change fingerprint as the new boundary, so even a loop without a verifier gets a next-iterate check that the revert actually restored the tree.
...
- `revertVerifier`: extension-executed rollback check (never run through the agent). At `multiloop_iterate` the extension runs it and captures the output as the pre-change workspace fingerprint — a content hash of the scope (e.g. `git rev-parse HEAD:$(git write-tree)`, or a sha256 of the changed files). At `multiloop_decide` with action `revert` it runs the command again and requires the fingerprint to be reproduced exactly: a 64-hex token in the current output must equal the captured one, byte-for-byte, no tolerance. The boundary is hashed at loop start and after every recorded decision (keep/revert/log), so the very first iterate already has a reference: files edited between `multiloop_start` and the first `multiloop_iterate` are refused at the first iterate, not silently absorbed into iteration 1. A passing `revert-verifier` check is appended to the iteration's checks; a failing one refuses the revert — the workspace must be byte-equivalent to its pre-change state, on the extension's word, not the agent's, and not merely metric-equivalent (a coincidentally unchanged metric proves nothing). Legacy numeric verifiers (no hash) fall back to the tolerance comparison against the pre-iteration value. A verifier that cannot run refuses to start the iteration (stop-the-line). Pinned like the other verifier fields. Loops without a configured `revertVerifier` get the same refuse-at-decide parity via the built-in git fingerprint: at `multiloop_decide` with action `revert`, the tracked working tree must equal its pre-change fingerprint or the revert is refused (a `revert-verifier` check is recorded either way).
- `minMeasurements`: minimum measurements before keep/revert may be decided (default 1). When a keep-revert loop is undersampled, `multiloop_measure` degrades the recommended action to `log` — record progress, do not promote or roll back on noise. Raise this for noisy metrics: a single noisy run is a coin flip (agent-eval variance is dominated by seed noise), and "keep it if the score went up" re-applied across iterations is uncontrolled adaptive multiple testing. Deterministic metrics keep the default 1 so they are not forced into pointless repeat runs.
- `pinnedConfig`: frozen verifier/stop-condition fields (`verifyCommand`, `guardCommand`, `promptVerifier`, `auditVerifier`, `revertVerifier`, `minMeasurements`, `metricName`, `metricDirection`, `targetMetric`, `maxIterations`, `protectedPaths`) captured at start. `multiloop_iterate`/`measure`/`decide`/`log` compare the on-disk state against the pin and refuse loudly on drift — a loop must not be allowed to edit the rule that measures it.
...
| Stop condition reached | `state.iteration` reaches `maxIterations`, or `state.currentMetric` reaches `targetMetric`. | Auto-continuation is unbounded by default; a loop the user bounded at setup must stop without depending on the human noticing. | The loop is marked `completed` and drops out of auto-continue; start a new run to keep going. |
| Resume of an already-complete loop | `multiloop_resume` or `/multiloop resume` targets a loop whose stop condition is still met. | Resuming would grant one silent bonus iteration before re-completing, and would hand the agent a resume prompt that contradicts its own stop condition. | Start a new run, or raise `maxIterations`/`targetMetric` in the lane's `state.json`. Escalation-stopped loops carry no stop condition and remain resumable. |
...
| Field | Semantics |
|---|---|
| `maxIterations` | Positive integer. Fires when `state.iteration >= maxIterations`. |
| `targetMetric` | Fires when `currentMetric <= targetMetric` (direction `lower`) or `currentMetric >= targetMetric` (direction `higher`). Never fires while `currentMetric` is `null`. |
| neither | Loop runs until paused, stopped, or escalation-exhausted. This is the default. |

The iteration cap is evaluated first when both are configured. Escalation exhaustion takes precedence over both and yields `stopped` rather than `completed`, preserving the distinction between “gave up” and “goal reached”.

Because both fields live in `state.json` rather than in the setup conversation, the bound survives compaction and session restarts. `buildIterationContext` renders them into every resume prompt so the agent can see how much budget is left.
...
On `session_start`, pi-multiloop resolves loop mode and then either resumes or announces.

1. Replay the session branch for the most recent `multiloop-mode` entry scoped to this cwd (`latestModeDecision`).
...
### Loop mode vs the per-turn flag

| | `loopMode` | `loopTurnActive` |
|---|---|---|
| Lifetime | session-durable, replayed from the session branch | one turn |
| Set by | `multiloop_start`, `/multiloop resume`, `/multiloop on`, session-start arming | any `multiloop_*` tool, or user input classified `arm` |
| Cleared by | `/multiloop off`, stop, pause, a `suspend` message | every `agent_end` |
| Question it answers | “is this session looping?” | “did this turn do loop work?” |

`shouldQueueContinuation` requires both plus a running loop. Before loop mode existed, `loopTurnActive` alone decided whether a loop existed, so any slash command cleared it and a new session never resumed one.
