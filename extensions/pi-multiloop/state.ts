...
export function reconstructState(cwd: string, id: LaneId): LoopState | null {
  ...
  for (const result of results) {
    ...
    // keep and revert have already continued, so this covers the log family:
    // log, skip, crash, and blocked. applyLogIteration moves currentMetric for
    // any of them that carries a metric, and replay has to agree -- otherwise a
    // loop completed by a target on a blocked/crash iteration rebuilds as
    // incomplete and becomes resumable after a restart.
    if (result.metric !== undefined) {
      currentMetric = result.metric;
    }
  }
