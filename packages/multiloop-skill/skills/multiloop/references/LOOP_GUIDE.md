...
| Stop condition | `maxIterations`, `targetMetric`, or neither for “until interrupted”. Always ask; always pass the answer to `multiloop_start`. |
...
The loop auto-continues after every iteration until a stop condition fires, so the user's stop answer must be recorded in loop state — not just in the conversation, which compaction discards.

| User answer | `multiloop_start` argument |
|---|---|
| “until all TODO items are checked” | `targetMetric: 0` (with the `open_or_partial_items` metric, direction `lower`) |
| “cap at 10 iterations” | `maxIterations: 10` |
| “stop at 200ms” | `targetMetric: 200` (direction `lower`) |
| “until I stop it” | omit both |

`maxIterations` is a positive integer counted against completed iterations. `targetMetric` completes the loop when the current metric reaches it — `<=` for `lower`, `>=` for `higher`. When both are set, whichever fires first completes the loop. Escalation exhaustion still stops a loop independently of either.
...
Pass `maxIterations` and/or `targetMetric` whenever the user chose a bounded stop condition. Omitting them means the loop runs until paused, stopped, or escalation-exhausted.
