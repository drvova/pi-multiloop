...
| Capability | pi-autoresearch | codex-autoresearch | pi-multiloop (this) |
|---|---|---|---|
...
- **Spawn background processes.** In-process by default; detached mode ships via `packages/multiloop-run/bin/multiloop-run.mjs` (headless `pi -p --mode json` per iteration, poll-and-reap cadence).
