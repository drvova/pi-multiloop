...
- [x] Give the setup guide's stop-condition question somewhere to land: `maxIterations` and `targetMetric` on `multiloop_start` and the loop snapshot, evaluated by both terminal paths. Escalation only fires on failure, so a succeeding loop previously auto-continued without any bound.
