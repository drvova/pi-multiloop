...
/** Session-level intent to keep looping. Durable across turns and slash commands. */
let loopMode = false;
...
/**
 * Read the loops the registry still calls active and that reconstruct to a
 * running snapshot.
 *
 * Pure read so it can be tested without the extension runtime. A corrupt or
 * partial snapshot is skipped rather than thrown: one bad lane must not stop a
 * session from starting, and the user can still resume it explicitly.
 */
export function collectRunningLoops(cwd: string, attachedKeys: ReadonlySet<string>): LoopState[] {
  const found: LoopState[] = [];
  let registry;
  try {
  ...
  for (const entry of registry.loops) {
    ...
    if (attachedKeys.has(`${id.lane}/${id.runTag}`)) continue;

    let state: LoopState | null = null;
    try {
      state = reconstructState(cwd, id);
    } catch {
      continue;
    }
...
/** Load reconstructable running loops into the in-memory active set. */
function attachRunningLoops(cwd: string): LoopState[] {
  const attached = collectRunningLoops(cwd, new Set(activeStates.keys()));
  for (const state of attached) {
    activeStates.set(stateKey({ lane: state.lane, runTag: state.runTag }), state);
  }
  return attached;
}

/**
 * Release a loop that reached a terminal state from the in-memory active set and
 * describe why the loop did or did not continue. Shared by multiloop_decide and
 * multiloop_log so both terminal paths report identically.
 */
function finishIterationNotice(id: LaneId, state: LoopState): string {
  if (state.status === "running") {
    return "Loop is still running; pi-multiloop will continue to the next required action automatically.";
  }

  activeStates.delete(stateKey(id));

  if (state.status === "stopped") {
    return "Loop has been stopped due to escalation exhaustion.";
  }

  const stop = checkStopCondition(state);
  return stop
    ? `Loop complete — ${stop.message} Start a new run to keep going.`
    : `Loop is no longer running (status: ${state.status}); pi-multiloop will not auto-continue it.`;
}
...
export function buildSetupGuidePrompt(goalSeed?: string): string {
  return [
    ...
    "8. Record the stop condition as an argument, not just in the conversation: pass maxIterations for an iteration cap, targetMetric for a metric/checklist target (targetMetric=0 completes a punchlist), or omit both to run until paused/stopped. A running loop auto-continues, so an unrecorded bound does not exist.",
    "",
    "Confirmation format:",
    "**Proposed loop**",
    "- Target: ...",
    ...
    "- Scope/lane: ...",
    "- Stop condition: ... (state it as maxIterations=N, targetMetric=N, or run-until-stopped)",
    "",
    "**Need to confirm**",
...
export default function (pi: ExtensionAPI) {
  ...
  const recordModeDecision = (cwd: string, active: boolean): void => {
    loopMode = active;
    pi.appendEntry(MODE_ENTRY_TYPE, { version: 1, cwd: resolve(cwd), active });
  };

  const readModeDecision = (ctx: ExtensionContext | ExtensionCommandContext): boolean | null => {
    const entries = ctx.sessionManager.getBranch() as unknown as ModeEntryLike[];
    return latestModeDecision(entries, resolve(ctx.cwd));
  };
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    loopMode = shouldArmLoopMode({
      hasRunningLoopOnDisk: attached.length > 0 || runningStates().length > 0,
      recordedDecision: recorded,
    });

    if (loopMode && runningStates().length > 0) {
      // Deliberately does not arm loopTurnActive: the queued prompt is the
      // first turn's cause, and that turn must earn the next continuation by
      // calling a loop tool. Arming here would let a chat-only reply re-prompt
      // itself once for free.
      queueLoopAutoContinue(pi, ctx, "session-start");
      updateStatus(ctx);
      return;
    }
  ...
  pi.on("input", async (event) => {
    ...
    switch (classifyUserInput(event.text)) {
      case "suspend":
        loopMode = false;
      ...
      case "arm":
        ...
        // attached loop would let an ordinary chat turn queue a continuation.
        if (runningStates().length > 0) markLoopTurn("user-query");
        break;
      case "neutral":
        // Slash commands drive the extension rather than abandon the loop.
        break;
    }
  });
  ...
  function startLoop(ctx: ExtensionContext | ExtensionCommandContext, config: StartLoopConfig): LoopState {
    ...
    recordModeDecision(ctx.cwd, true);
    updateStatus(ctx);
    return state;
  }
  ...
  const StartParams = Type.Object({
    ...
    targetMetric: Type.Optional(Type.Number({
      description: "Stop condition: complete the loop when the metric reaches this value (<= for lower-is-better, >= for higher-is-better). Omit for no target.",
    })),
  });
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      if (state.baseline === null) {
        ...
        lines.push(
          "",
          baselineStop
            ? `Goal already satisfied at baseline — ${baselineStop.message} Loop marked complete without iterating; start a new run if you want to push further.`
            : "Baseline recorded. Start optimizing."
        );
        if (baselineStop) activeStates.delete(stateKey(id));
      ...
      const noiseAdvisory = singleMeasurementAdvisory(state, confidence.measurements);
      if (noiseAdvisory) {
        lines.push(`  Note: ${noiseAdvisory}`);
      }
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      lines.push("");
      lines.push(finishIterationNotice(id, state));
  ...
  function resumeLoop(ctx: ExtensionContext | ExtensionCommandContext, id: LaneId): ResumeOutcome {
    const state = reconstructState(ctx.cwd, id);
    if (!state) return { ok: false, reason: `No state found for ${formatLaneId(id)}.` };

    const refusal = resumeRefusalReason(state);
  ...
  function pauseLoop(ctx: ExtensionContext | ExtensionCommandContext, id: LaneId): string {
    ...
    // Checked after the delete so the count reflects this lane leaving.
    if (shouldDisarmAfterLaneOperation(runningStates().length)) {
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const outcome = resumeLoop(ctx, resolution.id);
  ...
  pi.registerCommand("multiloop", {
    ...
    async handler(args, ctx) {
      ...
      if (trimmed === "off") {
        ...
        ctx.ui.notify(
          running > 0
            ? `Loop mode OFF — ${running} loop(s) stay running on disk; resume with /multiloop resume <lane/run-tag>.`
            : "Loop mode OFF.",
          "info"
        );
        return;
      ...
      if (trimmed === "on") {
        const attached = attachRunningLoops(ctx.cwd);
        if (attached.length === 0 && runningStates().length === 0) {
          ctx.ui.notify("No running loop to resume. Run /multiloop guide to create one.", "info");
          return;
        }
        recordModeDecision(ctx.cwd, true);
        queueLoopAutoContinue(pi, ctx, "mode-on");
        updateStatus(ctx);
        return;
      }
      ...
      if (trimmed === "help") {
        pi.sendMessage({
          ...
          content: [
            ...
            "  on               Re-arm loop mode and continue the running loop",
            "  off              Stop auto-continuing without stopping the loop",
