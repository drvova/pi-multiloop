...
- Add bounded stop conditions: `multiloop_start` accepts `maxIterations` (positive integer) and `targetMetric`, both persisted on the loop snapshot. A loop that reaches either is marked `completed` and drops out of auto-continuation.
